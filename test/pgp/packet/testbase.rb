require 'pgp/armor'
require 'pgp/packet'


module TestPGP
module TestPacket


module TestBase
  pgp_asc_pub = <<__EOP__
-----BEGIN PGP PUBLIC KEY BLOCK-----


UIWur7ty3zA3NlYsB3xg+ebhIQynFGIdYmTh
=vxBn
-----END PGP PUBLIC KEY BLOCK-----
__EOP__

  pgp_asc_sec = <<__EOP__
-----BEGIN PGP PRIVATE KEY BLOCK-----


=CaT2
-----END PGP PRIVATE KEY BLOCK-----
__EOP__

  pgp_asc_msg = <<__EOP__
-----BEGIN PGP MESSAGE-----


=ffoS
-----END PGP MESSAGE-----
__EOP__

  PUB_KEY, PUB_UID, PUB_USIG, PUB_SUBKEY, PUB_SUBSIG =
    PGP::Packet::Packet.load(PGP::Armor.new(pgp_asc_pub).body)

  SEC_KEY, SEC_UID, SEC_SUBKEY =
    PGP::Packet::Packet.load(PGP::Armor.new(pgp_asc_sec).body)

  MSG_SESSKEY, MSG_DATA =
    PGP::Packet::Packet.load(PGP::Armor.new(pgp_asc_msg).body)
end


end
end
